...
from pointcept.models.peft import DynamicParameterModule
...
class Block(PointModule):
    def __init__(
        ...
        self.cur_layer = check_set[stage] + layer
        self.dy_down = False
        self.dy_up = False
        self.is_enc = is_encoder

        if self.is_enc:
            if self.cur_layer in dy_down_layers:
                self.dy_down = True

            if self.cur_layer in dy_up_layers:
                self.dy_up = True

            self.adaptive_module = DynamicParameterModule(
                in_channel=channels, mid=mid, K=K, param=param, 
                dy_down=self.dy_down, stage=stage, layer=layer, mode=mode,
                dy_up=self.dy_up, bias=True, order_idx=order_index)
...
@MODELS.register_module("PTV3-PointTPA")
class PointTransformerV3(PointModule):
    def __init__(
        ...
        dy_down_layers=[2, 5, 8, 20, 23],
        dy_up_layers=[],
        K_list=[2, 2, 2, 2, 2],
        param_list=[200, 100, 50, 20, 10],
        mode_list=["num", "num", "num", "num", "num"],
        adaptive_mid=64,
        mlp_ratio=4,
        qkv_bias=True,
        qk_scale=None,
            ...
            for i in range(enc_depths[s]):
                enc.add(
                    Block(
                        ...
                        dy_down_layers=dy_down_layers,
                        dy_up_layers=dy_up_layers,
                        stage=s,
                        layer=i,
                        param=param_list[s],
                        mode=mode_list[s],
